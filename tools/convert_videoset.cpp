...
DEFINE_int32(new_length, 16, "Length of a video segment feeding into data layer");
...
int main(int argc, char** argv) {
  ...
  const int sampling_rate = FLAGS_sampling_rate;
  ...
  for (int line_id = 0; line_id < lines.size(); ++line_id) {
    ...
    status = ReadImageSequenceToVolumeDatum(lines[line_id].first.c_str(), lines[line_id].second.first,
                                            lines[line_id].second.second, new_length, resize_height, resize_width, sampling_rate,
                                            &datum);
