...
void* VideoDataLayerPrefetch(void* layer_pointer) {
    ...
    const Dtype scale = layer->layer_param_.video_data_param().scale();
    const int batch_size = layer->layer_param_.video_data_param().batch_size();
    const int crop_size = layer->layer_param_.video_data_param().crop_size();
    const bool mirror = layer->layer_param_.video_data_param().mirror();
    const int new_length  = layer->layer_param_.video_data_param().new_length();
    const int new_height  = layer->layer_param_.video_data_param().new_height();
    const int new_width  = layer->layer_param_.video_data_param().new_width();
    const bool use_image = layer->layer_param_.video_data_param().use_image();
    const int sampling_rate = layer->layer_param_.video_data_param().sampling_rate();
    const bool use_temporal_jitter = layer->layer_param_.video_data_param().use_temporal_jitter();
    ...
    const int show_data = layer->layer_param_.video_data_param().show_data();
    char *data_buffer;
    if (show_data)
...
template <typename Dtype>
void VideoDataLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
    ...
    if (crop_size > 0) {
        newshape[0] = this->layer_param_.video_data_param().batch_size();
...
void VideoDataLayer<Dtype>::CreatePrefetchThread() {
    ...
    const bool prefetch_needs_rand = (this->phase_ == caffe::TRAIN) &&
            (this->layer_param_.video_data_param().mirror() ||
             this->layer_param_.video_data_param().crop_size());
